...
def main():
    ...
    potential_locations = []
    for station_data, label in zip(X_test, y_test):
        if label == 1:  # Si la ubicación es etiquetada como potencial
            lat = station_data[0]
            lon = station_data[1]
            potential_locations.append((lat, lon))
    ...
    # Crear un mapa interactivo con las ubicaciones potenciales dentro del estado de Durango
    create_interactive_map(filtered_potential_locations)
